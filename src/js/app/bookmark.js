...
define(function () {
  const makeBookmarkFromJson = ({ id, request, name, folder, created }) => ({
    id,
    request,
    name,
    folder,
  ...
  const makeFolderFromJson = ({ id, name, bookmarks = [], created }) => ({
    id,
    name,
    bookmarks,
    isFolder: true,
  ...
  const makeBookmark = (id, request, name, folder, created = new Date()) => ({
    id,
    request,
    name,
    folder,
  ...
  const makeFolder = (id, name, bookmarks = [], created = new Date()) => ({
    id,
    name,
    bookmarks,
    isFolder: true,
  ...
  const removeBookmarks = (folder, bookmarksToRemove = []) => {
    ...
    bookmarks = bookmarks.filter(
      (b) => bookmarksToRemoveIds.indexOf(b.id) === -1
    )
  ...
  const importHAR = (storageProvider, harContent) => (harContent) => {
    ...
    if (_isRestingFormat(har.log.creator)) {
      ...
      const bookmarks = harEntries.map((entry) =>
        _convertHarEntry(storageProvider, entry)
      )
  ...
  const _isRestingFormat = (creatorFields = {}) => {
    return (
      creatorFields.name == 'Resting WebExtension' &&
      creatorFields.version == '1.0'
    )
  }
  ...
  const _convertHarRequest = (harRequest = {}) => {
    ...
    if (harRequest.url) {
      ...
      const endUrlIndex =
        querystringIndex != -1 ? querystringIndex : harRequest.url.length
      request.url = harRequest.url.substring(0, endUrlIndex)
    }
    request.method = harRequest.method
    if (harRequest.queryString) {
      request.querystring = harRequest.queryString.map((qs) => ({
        name: qs.name,
        value: qs.value,
      }))
    }
    if (harRequest.headers) {
      request.headers = harRequest.headers.map((header) => ({
        name: header.name,
        value: header.value,
      }))
    }
    if (harRequest.postData) {
      request.headers.push({
        name: 'Content-Type',
        value: harRequest.postData.mimeType,
      })
    ...
    if (sources.length > 0) {
      ...
      if (sources[0].request) {
        ...
        if (sources[0].request.headers) {
          bookmarkExport.request.headers = sources[0].request.headers.map(
            (h) => ({ name: h.name, value: h.value, _enabled: h.enabled })
          )
        } else {
        ...
        if (sources[0].request.querystring) {
          bookmarkExport.request.queryString =
            sources[0].request.querystring.map((q) => ({
              name: q.name,
              value: q.value,
              _enabled: q.enabled,
            }))
        ...
        if (sources[0].request.body) {
          ...
          bookmarkExport.request.postData.mimeType = _getMimeType(
            sources[0].request.bodyType
          )
          if (
            bookmarkExport.request.postData.mimeType ===
              'application/x-www-form-urlencoded' ||
            bookmarkExport.request.postData.mimeType === 'multipart/form-data'
          ) {
            bookmarkExport.request.postData.params =
              sources[0].request.body.map((p) => ({
                name: p.name,
                value: p.value,
                _enabled: p.enabled,
              }))
        ...
        bookmarkExport.request._authentication =
          sources[0].request.authentication
  ...
  const importObj = (obj = {}) => {
    ...
    if (entries) {
      ...
      importObj.bookmarks = _fixStructure(
        importObj.bookmarks,
        indexRelationship
      )
    }
  ...
  const _fixStructure = (bookmarks, mapping) => {
    ...
    for (let folderId of Object.keys(mapping)) {
      ...
      insideBookmarkIds.forEach((id) => {
        if (
          indexBookmarks.hasOwnProperty(folderId) &&
  ...
  const _importEntry = (entry = {}) => {
    ...
    if (entryRequest.headers) {
      bookmark.request.headers = entryRequest.headers.map((h) => ({
        name: h.name,
        value: h.value,
        enabled: h._enabled,
      }))
    ...
    if (entryRequest.queryString) {
      bookmark.request.querystring = entryRequest.queryString.map((h) => ({
        name: h.name,
        value: h.value,
        enabled: h._enabled,
      }))
    ...
    if (entryRequest.postData) {
      ...
      if (
        entryRequest.postData.mimeType === 'multipart/form-data' ||
        entryRequest.postData.mimeType === 'application/x-www-form-urlencoded'
      ) {
        bookmark.request.body = entryRequest.postData.params.map((p) => ({
          name: p.name,
          value: p.value,
          enabled: p._enabled,
        }))
  ...
  const _importContext = (entry = {}) => {
    ...
    if (entry.variables) {
      context.variables = entry.variables.map((v) => ({
        name: v.name,
        value: v.value,
        enabled: v.enabled,
      }))
