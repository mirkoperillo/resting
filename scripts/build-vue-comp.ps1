...
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name entry-list-item  "$COMPONENTS_FOLDER/EntryListItem.vue"
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name entry-list  "$COMPONENTS_FOLDER/EntryList.vue"
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name context-dialog  "$COMPONENTS_FOLDER/ContextDialog.vue"
& "$CLI_PATH/vue-cli-service.ps1" build --target lib --formats umd --dest $DIST_FOLDER --no-clean --name confirm-dialog  "$COMPONENTS_FOLDER/ConfirmDialog.vue"